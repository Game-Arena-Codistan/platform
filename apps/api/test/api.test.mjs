...
test('health and catalogue are public',async()=>{const f=await fixture();try{assert.equal((await fetch(`${f.base}/healthz`)).status,200);const data=await (await fetch(`${f.base}/v1/catalog/games`)).json();assert.ok(data.games.length>=5);}finally{await f.close();}});
