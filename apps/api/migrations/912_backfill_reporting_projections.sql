...
SELECT record->>'version',record->>'id',record->>'name',COALESCE(record->>'status','active'),
  ...
  NULLIF(record->>'effectiveAt','')::timestamptz,NULLIF(record->>'scheduledAt','')::timestamptz,
  NULLIF(record->>'retiredAt','')::timestamptz,record,clock_timestamp()
...
SELECT (record->>'id')::uuid,(record->>'userId')::uuid,COALESCE(record->>'kind','membership'),
  ...
  COALESCE(NULLIF(record->>'createdAt','')::timestamptz,clock_timestamp()),
  COALESCE(NULLIF(record->>'initiatedAt','')::timestamptz,NULLIF(record->>'createdAt','')::timestamptz),
  COALESCE(NULLIF(record->>'completedAt','')::timestamptz,NULLIF(record->>'paidAt','')::timestamptz),
  NULLIF(record->>'failedAt','')::timestamptz,NULLIF(record->>'cancelledAt','')::timestamptz,
  NULLIF(record->>'refundedAt','')::timestamptz,
  COALESCE(NULLIF(record->>'updatedAt','')::timestamptz,NULLIF(record->>'createdAt','')::timestamptz,clock_timestamp()),record
...
SELECT COALESCE(NULLIF(record->>'id',''),record_key)::uuid,(record->>'transactionId')::uuid,
  ...
  COALESCE((record->>'signatureValid')::boolean,false),COALESCE(NULLIF(record->>'receivedAt','')::timestamptz,clock_timestamp()),
  NULLIF(record->>'processedAt','')::timestamptz,COALESCE((record->>'duplicate')::boolean,false),record->>'protectedEvidenceReference'
...
WITH latest AS (
  ...
  ORDER BY record->>'id',COALESCE(record->>'updatedAt',record->>'createdAt','') DESC
)
...
SELECT (record->>'id')::uuid,(record->>'userId')::uuid,NULLIF(record->>'planId',''),record->'planSnapshot',
  ...
  COALESCE(NULLIF(record->>'startsAt','')::timestamptz,NULLIF(record->>'createdAt','')::timestamptz),
  COALESCE(NULLIF(record->>'currentPeriodStartsAt','')::timestamptz,NULLIF(record->>'startsAt','')::timestamptz),
  COALESCE(NULLIF(record->>'currentPeriodEndsAt','')::timestamptz,NULLIF(record->>'expiresAt','')::timestamptz),
  NULLIF(record->>'expiresAt','')::timestamptz,NULLIF(record->>'graceEndsAt','')::timestamptz,
  NULLIF(record->>'cancelledAt','')::timestamptz,NULLIF(record->>'nextRenewalAt','')::timestamptz,
...
SELECT (record->>'id')::uuid,NULLIF(record->>'transactionId','')::uuid,record->>'reason',COALESCE(record->>'status','open'),
  COALESCE(NULLIF(record->>'createdAt','')::timestamptz,clock_timestamp()),NULLIF(record->>'updatedAt','')::timestamptz,
...
SELECT (record->>'id')::uuid,record->>'actor',record->>'reportType',
  ...
  record->>'contentHash',COALESCE(NULLIF(record->>'generatedAt','')::timestamptz,clock_timestamp())
...
SELECT (record->>'id')::uuid,COALESCE(NULLIF(record->>'at','')::timestamptz,clock_timestamp()),record->>'actor',
