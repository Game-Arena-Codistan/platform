...
const otpDelivery=new OtpDeliveryRouter({providers,audit:store.audit,metrics:store.metrics});const jazzcash=new JazzCashAdapter(config);const payments=new PaymentService({store,provider:jazzcash});const rewardPolicy=new RewardPolicy({config,store});const mvp=createMvpApp({config,store,payments});const primary=createApp({config,store,jazzcash,otpDelivery,payments,rewardPolicy});const supplemental=createSupplementalApp({config,store});
const server=createServer(async(req,res)=>{res.once('finish',()=>store.markDirty?.());try{if(await mvp(req,res)!==false)return;if(await supplemental(req,res)!==false)return;await primary(req,res);}catch(error){console.error(JSON.stringify({level:'error',message:'Unhandled request error',error:error.message}));if(!res.headersSent){res.writeHead(500,{'content-type':'application/json'});res.end(JSON.stringify({error:{code:'internal_error',message:'Unexpected server error.'}}));}}});
