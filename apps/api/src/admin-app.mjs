...
import {requestIp,safeEqual,sha256} from './lib/security.mjs';
...
export function createSupplementalApp({config,store}){
  function admin(req,roles){const key=String(req.headers['x-admin-key']||'');if(!key||!config.adminApiKeys.some(candidate=>safeEqual(candidate,key)))throw fail('Administrative authentication required.',401,'admin_authentication_required');const role=String(req.headers['x-admin-role']||'admin');if(!roles.includes(role)&&!roles.includes('any'))throw fail('Administrative role is not permitted.',403,'admin_forbidden');return{actor:`admin:${sha256(key).slice(0,12)}`,role};}
