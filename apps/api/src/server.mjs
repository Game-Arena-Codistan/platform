...
const server=createServer(async(req,res)=>{res.once('finish',()=>store.markDirty?.());try{if(await supplemental(req,res)!==false)return;await primary(req,res);}catch(error){console.error(JSON.stringify({level:'error',message:'Unhandled request error',error:error.message}));if(!res.headersSent){res.writeHead(500,{'content-type':'application/json'});res.end(JSON.stringify({error:{code:'internal_error',message:'Unexpected server error.'}}));}}});
