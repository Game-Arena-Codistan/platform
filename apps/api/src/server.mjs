...
const config=loadConfig();let store;
if(config.databaseUrl){const {PostgresStore}=await import('./adapters/postgres-store.mjs');store=await PostgresStore.connect({connectionString:config.databaseUrl,ssl:config.databaseSsl});}
else{if(config.requireDatabase)throw new Error('DATABASE_URL is required in production.');store=new MemoryStore();}
...
const otpDelivery=new OtpDeliveryRouter({providers,audit:store.audit,metrics:store.metrics});const jazzcash=new JazzCashAdapter(config);const payments=new PaymentService({store,provider:jazzcash});const rewardPolicy=new RewardPolicy({config,store});const app=createApp({config,store,jazzcash,otpDelivery,payments,rewardPolicy});const server=createServer(app);
