...
const MAPS=[
  ['users','ga_runtime_users'],['identities','ga_runtime_identities'],['otp','ga_runtime_otp'],
  ['sessions','ga_runtime_sessions'],['devices','ga_runtime_devices'],['entitlements','ga_runtime_entitlements'],
  ['transactions','ga_runtime_transactions'],['paymentEvents','ga_runtime_payment_events'],
  ['playSessions','ga_runtime_play_sessions'],['rateLimits','ga_runtime_rate_limits'],
  ['challenges','ga_runtime_challenges'],['tournaments','ga_runtime_tournaments'],
  ['adjustments','ga_runtime_adjustments'],['multiplayerRooms','ga_runtime_multiplayer_rooms']
];
const ARRAYS=[
  ['entitlementHistory','ga_runtime_entitlement_history',item=>`${item.id}:${item.updatedAt??item.createdAt}`],
  ['reconciliationCases','ga_runtime_reconciliation_cases',item=>item.id],
  ['ledger','ga_runtime_coin_ledger',item=>item.id],['games','ga_runtime_games',item=>item.id],
...
const DESCRIPTORS=[
  ...MAPS.map(([name,table])=>({name,table,kind:'map',keyOf:(_record,key)=>key})),
  ...ARRAYS.map(([name,table,keyOf])=>({name,table,kind:'array',keyOf}))
];
const BY_NAME=new Map(DESCRIPTORS.map(item=>[item.name,item]));
...
function container(store,descriptor){return descriptor.name==='auditEvents'?store.audit.events:store[descriptor.name];}
function clear(store,descriptor){
...
function records(store,descriptor){
...
function apply(store,descriptor,key,record,deleted){
...
function sortArrays(store){
  const sort=(field,numeric=false)=>(a,b)=>{
    const av=numeric?Number(a?.[field]||0):(Date.parse(a?.[field]||0)||Number(a?.[field]||0));
    const bv=numeric?Number(b?.[field]||0):(Date.parse(b?.[field]||0)||Number(b?.[field]||0));
    return av-bv||String(a?.id||a?.playSessionId||'').localeCompare(String(b?.id||b?.playSessionId||''));
  };
  store.entitlementHistory.sort(sort('createdAt'));
  store.reconciliationCases.sort(sort('createdAt'));store.ledger.sort(sort('createdAt'));
  store.scoreEvents.sort(sort('completedAt',true));store.tournamentEntries.sort(sort('joinedAt',true));
  store.supportTickets.sort(sort('createdAt'));store.voucherRedemptions.sort(sort('redeemedAt'));
  store.planVersions.sort((a,b)=>String(a.id).localeCompare(String(b.id))||String(a.version).localeCompare(String(b.version)));
  store.benefitLedger.sort(sort('createdAt'));store.reportExports.sort(sort('generatedAt'));
  store.audit.events.sort(sort('at'));
...
function conflict(collection,key){
  return Object.assign(new Error(`Concurrent PostgreSQL update detected for ${collection}/${key}. Retry after refreshing committed state.`),{code:'persistence_conflict',status:409});
}

async function project(client,{collection,key,record,deleted}){
  ...
  if(collection==='entitlementHistory'){
    if(deleted||record.tier!=='premium')return;
...
export class PostgresStore extends MemoryStore{
  static async connect({connectionString,ssl=false,ca=''}){
    ...
    if(model.rows[0]?.name!==NORMALIZED_POSTGRES_MODEL){await pool.end();throw new Error('Normalized PostgreSQL migrations are not applied. Run the migration job before starting the API.');}
    const legacy=await pool.query("SELECT to_regclass('public.platform_state') AS table_name");
    if(legacy.rows[0]?.table_name){await pool.end();throw new Error('Legacy platform state still exists. Apply migration 911 before starting the API.');}

    const store=new PostgresStore({pool});
    const defaults={
      games:clone(store.games),
      challenges:new Map([...store.challenges].map(([key,value])=>[key,clone(value)])),
      tournaments:new Map([...store.tournaments].map(([key,value])=>[key,clone(value)]))
    };
    for(const descriptor of DESCRIPTORS)clear(store,descriptor);
    await store.refresh({initial:true});
    if(!store.games.length)store.games=defaults.games;
    if(!store.challenges.size)store.challenges=defaults.challenges;
    if(!store.tournaments.size)store.tournaments=defaults.tournaments;
    rebuild(store);
    await store.commit();
    await store.refresh({initial:true});
    return store;
  ...
  constructor({pool}){
    ...
    this.pool=pool;this.persistenceModel=NORMALIZED_POSTGRES_MODEL;
  ...
  runExclusive(task){
    const run=this.requestQueue.then(task,task);this.requestQueue=run.catch(()=>{});return run;
  }
  ...
  async refresh({initial=false}={}){
    ...
    try{
      ...
      const union=DESCRIPTORS.map(descriptor=>`SELECT '${descriptor.name}' AS collection,record_key,revision,record,deleted_at,updated_at FROM ${descriptor.table} WHERE updated_at >= $1`).join(' UNION ALL ');
      const result=await client.query(`${union} ORDER BY updated_at,collection,record_key`,[since]);
      for(const row of result.rows){
        const descriptor=BY_NAME.get(row.collection);if(!descriptor)continue;
        const key=String(row.record_key);const deleted=Boolean(row.deleted_at);
        apply(this,descriptor,key,row.record,deleted);
      ...
      await client.query('COMMIT');this.lastRefreshAt=cutoff;rebuild(this);
  ...
  collectChanges(){
    ...
    for(const descriptor of DESCRIPTORS){
      const current=records(this,descriptor);const snapshots=this.snapshots.get(descriptor.name);const revisions=this.revisions.get(descriptor.name);
      for(const [key,record] of current){
        ...
        if(previous!==next)changes.push({collection:descriptor.name,descriptor,key,record:clone(record),deleted:false,expectedRevision:revisions.get(key)});
      }
      for(const [key,previous] of snapshots){
        if(previous!==null&&!current.has(key))changes.push({collection:descriptor.name,descriptor,key,record:null,deleted:true,expectedRevision:revisions.get(key)});
      }
    }
    return changes;
  }
  ...
  async commit(){
    ...
    try{
      ...
      for(const change of changes){
        const {descriptor,key,record,deleted,expectedRevision}=change;let result;
        ...
        await project(client,change);committed.push(change);
      }
