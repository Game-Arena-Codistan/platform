...
const required=['index.html','styles/tokens.css','styles/app.css','styles/responsive.css','manifest.webmanifest','sw.js','src/app.js','src/data.js','src/catalogue-runtime.js','src/state.js','src/api.js','src/ui.js','src/analytics.js','src/game-bridge.js','src/views/feed.js','src/views/library.js','src/views/rewards.js','src/views/premium.js','src/views/account.js','assets/icon.svg','assets/logo.svg','legal/privacy.html','legal/terms.html','deploy/nginx.conf','deploy/40-game-arena-config.sh','Dockerfile','_headers','vercel.json','scripts/build.mjs','demo-games/arena-dash/index.html'];
...
const budgetFiles=['index.html','styles/tokens.css','styles/app.css','styles/responsive.css',...scripts.map(file=>relative(root,file))]
  .filter(file=>!/(^|[\\/])catalogue-(1|2|art)\.js$/.test(file));
const bytes=(await Promise.all(budgetFiles.map(file=>stat(join(root,file))))).reduce((sum,item)=>sum+item.size,0);
if(bytes>110*1024)throw new Error(`Core source ${(bytes/1024).toFixed(1)} KB exceeds 110 KB.`);
