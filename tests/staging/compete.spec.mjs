...
test('@player authenticated user can create and rejoin a supported multiplayer room',async({page},testInfo)=>{
  ...
  await expect(page.getByRole('dialog',{name:/Create multiplayer room/i})).toBeVisible();
  const name=`QA ${runId.slice(-12)}`;
  await page.locator('#room-name').fill(name);
  const gameOptions=await page.locator('#room-game option').count();
  test.skip(gameOptions===0,'No multiplayer-capable game is exposed to the deployed player.');
  await page.locator('#room-size').selectOption('2');
