...
const requestedWait=Number(process.env.QA_MAX_WAIT_MS||180000);
const maxWaitMs=Number.isFinite(requestedWait)?Math.min(240000,Math.max(30000,requestedWait)):180000;
...
let delayMs=2000;
let polls=0;
let state='BREVO_OTP_NOT_VISIBLE';
let lastHttp=0;
while(Date.now()<deadline){
  ...
  try{
    ...
    if(!listResponse.ok){
      lastHttp=listResponse.status;
      if(listResponse.status===429||listResponse.status>=500){
        state='BREVO_OTP_PROVIDER_RETRYABLE';
        await pause(delayMs);
    ...
    for(const item of list.transactionalEmails||[]){
      ...
      state='BREVO_OTP_DELIVERY_PENDING';
      ...
      if(!contentResponse.ok){
        lastHttp=contentResponse.status;
        if(contentResponse.status===404){state='BREVO_OTP_CONTENT_PENDING';continue;}
        if(contentResponse.status===429||contentResponse.status>=500){state='BREVO_OTP_PROVIDER_RETRYABLE';break;}
...
function runSsh(args,input,{timeout=210000,maxBuffer=4096}={}){
...
export async function fetchDeliveredBrevoOtp(identity,{notBefore=Date.now()-5000,waitMs=180000}={}){
  ...
  const boundedWait=Math.min(240000,Math.max(30000,Number(waitMs)||180000));
  ...
  const remote=`cd /opt/codistan/platform && IMAGE_BASE=${imageBase} IMAGE_TAG=${imageTag} docker compose -f infra/docker-compose.staging.yml --env-file infra/.env exec -T -e QA_IDENTITY_B64=${identityB64} -e QA_NOT_BEFORE=${Math.floor(notBefore)} -e QA_MAX_WAIT_MS=${boundedWait} api node --input-type=module -`;
  try{
    const stdout=await runSsh(['-i',key,'-o','BatchMode=yes',`${user}@${host}`,remote],remoteScript,{timeout:boundedWait+30000});
    const code=String(stdout||'').trim();
    if(!/^\d{6}$/.test(code))throw new Error('BREVO_OTP_INVALID_REMOTE_RESPONSE');
    return code;
  }catch(error){
    const raw=String(error?.message||'BREVO_OTP_UNKNOWN');
    const marker=raw.match(/BREVO_OTP_[A-Z0-9_]+/)?.[0]||'BREVO_OTP_UNKNOWN';
    const diagnostics=raw.match(/elapsedMs=\d+;polls=\d+(?:;lastHttp=\d+)?/)?.[0]||'';
    throw new Error(`BLOCKED: delivered Brevo OTP verification failed (${marker}${diagnostics?`; ${diagnostics}`:''}).`);
  }
}
