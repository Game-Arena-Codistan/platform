...
if [ -n "${RUNNER_NAME:-}" ] && [ "${RUNNER_OS:-}" != Windows ]; then
  fail "GitHub reports RUNNER_OS=${RUNNER_OS}; Game Arena's dedicated runner must use Windows."
fi
for tool in git node npm docker jq curl tofu bash; do
  command -v "$tool" >/dev/null 2>&1 || fail "Required runner tool is missing: $tool"
done
node_major="$(node -p "process.versions.node.split('.')[0]")"
[ "$node_major" = 22 ] || fail "Node.js major 22 is required; found $(node --version)."
docker info >/dev/null 2>&1 || fail 'Docker Desktop Linux-container engine is unavailable.'
...
echo "Runner: ${RUNNER_NAME:-manual-check}"
