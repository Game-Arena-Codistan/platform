...
const required=[
  ...
  'apps/web/vercel.json','apps/api/scripts/bootstrap-db-role.mjs','infra/kubernetes/platform.yaml','infra/kubernetes/admin.yaml','infra/kubernetes/migration-job.yaml','infra/kubernetes/edge-aws.yaml','infra/opentofu/aws/main.tf','infra/opentofu/aws/variables.tf','infra/opentofu/aws/outputs.tf','infra/opentofu/aws/game-delivery.tf','infra/opentofu/aws/operations.tf','infra/opentofu/aws/runtime-secrets.tf','infra/opentofu/aws/runtime-access.tf','scripts/render-kubernetes.mjs'
];
for(const file of required)await access(join(root,file));
for(const obsolete of ['.github/workflows/deploy-kubernetes.yml','.github/workflows/infrastructure-opentofu.yml','infra/kubernetes/edge-digitalocean.yaml','.github/workflows/one-shot-launch-patches.yml','.github/workflows/one-shot-launch-patches-v2.yml']){try{await access(join(root,obsolete));throw new Error(`Obsolete deployment path is still active: ${obsolete}`);}catch(error){if(error?.code!=='ENOENT')throw error;}}

const env={...process.env,IMAGE_TAG:'0123456789abcdef0123456789abcdef01234567',DEPLOY_ENVIRONMENT:'staging',PUBLIC_HOST:'preview.example.com',GAME_HOST:'games.example.com',PUBLIC_ORIGIN:'https://preview.example.com',ALLOWED_ORIGINS:'https://preview.example.com',GAME_ORIGIN:'https://games.example.com/games',GAME_HOSTS:'games.example.com',DATABASE_SSL:'true',OTP_PROVIDER_MODE:'mock',ALLOW_DEBUG_OTP:'true',JAZZCASH_MODE:'mock',AWS_CERTIFICATE_ARN:'arn:aws:acm:ap-south-1:123456789012:certificate/example',ECR_REGISTRY:'123456789012.dkr.ecr.ap-south-1.amazonaws.com',ECR_PREFIX:'game-arena/staging'};
...
try{
  ...
  if(!awsProduction.includes('provider-guard')||!awsProduction.includes('require_staging_evidence: true')||!awsProduction.includes('runtime-controls')||!awsProduction.includes('confirmation must be PROMOTE.'))throw new Error('Production promotion must require provider, staging, runtime and explicit confirmation guards.');
  ...
  for(const marker of ['bootstrap-db-role.mjs','DATABASE_APP_PASSWORD','ADMIN_PROXY_SECRET','SUPPORT_DELIVERY_ENDPOINT','wafv2-acl-arn','access_logs.s3.enabled=true','GAME_ARENA_ARTIFACT_ORIGIN'])if(!runtimeControls.includes(marker))throw new Error(`AWS runtime controls are missing ${marker}`);
  ...
  if(!runtimeSecrets.includes('database-application')||!runtimeSecrets.includes('runtime-controls'))throw new Error('AWS must provision separate application database and runtime control secrets.');
  if(!runtimeAccess.includes('AmazonEKSAdminPolicy')||!runtimeAccess.includes('namespaces = ["game-arena"]'))throw new Error('AWS runtime access must be namespace scoped.');
