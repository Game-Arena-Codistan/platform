...
for(const file of files){
  ...
  if(linuxSelector.test(content))failures.push(`${file}: Linux runner selector remains; Game Arena uses the existing Windows host`);
}
...
if(failures.length){
  ...
  console.log(`Validated ${files.length} workflows: all jobs use the Windows self-hosted runner policy.`);
}
